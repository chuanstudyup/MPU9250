...
struct MPU9250Setting {
    ...
    uint8_t gyro_fchoice {0x00};//0x03
    GYRO_DLPF_CFG gyro_dlpf_cfg {GYRO_DLPF_CFG::DLPF_41HZ};
    uint8_t accel_fchoice {0x00};//0x01
    ACCEL_DLPF_CFG accel_dlpf_cfg {ACCEL_DLPF_CFG::DLPF_41HZ};
};
...
class MPU9250 {
    ...
    float magnetic_declination = -4.73;  // Deg @Wuhan,China
...
public:
    ...
    float Ta12,Ta13,Ta23;
    float Ka11,Ka22,Ka33;
    float Bax,Bay,Baz;  //单位： g
    
    //cal_gyro = TgKg(raw_gyrp+Bg)
    float Tg12,Tg13,Tg21,Tg23,Tg31,Tg32;
    float Kg11,Kg22,Kg33;
    float Bgx,Bgy,Bgz;  //单位： deg/s
    
    //cal_mag = Kg(raw_mag-Bg)
    ...
    bool setup(const uint8_t addr, const MPU9250Setting& mpu_setting = MPU9250Setting()) {
        ...
        getCalibrationParam(acc_bias,gyro_bias,mag_bias,mag_scale);
        has_connected = true;
        return true;
    }
    ...
    void initRecordCalData()
    {
        Ta12=0;Ta13=0;Ta23=0;
        Ka11=1;Ka22=1;Ka33=1;
        Bax=0;Bay=0;Baz=0;
        
        Tg12=0;Tg13=0;Tg21=0;Tg23=0;Tg31=0;Tg32=0;
        Kg11=1;Kg22=1;Kg33=1;
        Bgx=0;Bgy=0;Bgz=0;
        
        setMagBias(0,0,0,false);
        setMagScale(1,1,1,false);
    }
    
    bool coarseAlignment(){
        ...
        float _roll = atan(ae/ad);
        float _pitch = -atan(an/sqrt(ae*ae+ad*ad));
        
        float hx = mn*cos(_pitch)+me*sin(_pitch)*sin(_roll)+md*cos(_roll)*sin(_pitch);
        float hy = me*cos(_roll)-md*sin(_roll);
        float _yaw = -atan2f(hy,hx) + magnetic_declination*DEG_TO_RAD;
        if(_yaw < -M_PI)
            _yaw += 2*M_PI;
        
        update_quat(_roll,_pitch,_yaw);
    ...
    void update_quat(float roll, float pitch, float yaw){
    ...
    void setMagBias(const float x, const float y, const float z, bool save) {
        mag_bias[0] = x;
        mag_bias[1] = y;
        mag_bias[2] = z;
        if(save)
            saveMagBias(mag_bias);
    }
    void setMagScale(const float x, const float y, const float z, bool save) {
        mag_scale[0] = x;
        mag_scale[1] = y;
        mag_scale[2] = z;
        if(save)
            saveMagScale(mag_scale);
    }
    ...
    void print_calibration() {
        ...
        printf("accel bias [mg]: \n");
        printf("%f, %f, %f\n",
                getAccBiasX() * 1000.f / (float)MPU9250::CALIB_ACCEL_SENSITIVITY,
                getAccBiasY() * 1000.f / (float)MPU9250::CALIB_ACCEL_SENSITIVITY,
                getAccBiasZ() * 1000.f / (float)MPU9250::CALIB_ACCEL_SENSITIVITY);
        
        printf("gyro bias [deg/s]: \n");
        printf("%f, %f, %f\n",
                getGyroBiasX() / (float)MPU9250::CALIB_GYRO_SENSITIVITY,
                getGyroBiasY() / (float)MPU9250::CALIB_GYRO_SENSITIVITY,
                getGyroBiasZ() / (float)MPU9250::CALIB_GYRO_SENSITIVITY);
...
private:
    ...
    void update_accel_gyro() {
        int16_t raw_acc_gyro_data[7]={0};        // used to read all 14 bytes at once from the MPU9250 accel/gyro
        ...
        float _ax = Ka11*(a[0]+Bax);
        float _ay = Ka22*(a[1]+Bay);
        float _az = Ka33*(a[2]+Baz);
        a[0] = _ax + Ta12*_ay + Ta13*_az;
        a[1] = _ay + Ta23*_az;
        a[2] = _az;
        ...
        float _gx = Kg11*(g[0]+Bgx);
        float _gy = Kg22*(g[1]+Bgy);
        float _gz = Kg33*(g[2]+Bgz);
        
        g[0] = _gx + Tg12*_gy + Tg13*_gz;
        g[1] = Tg21*_gx + _gy + Tg23*_gz;
        g[2] = Tg31*_gx + Tg32*_gy + _gz;
    }

    void read_accel_gyro(int16_t* destination) {
        uint8_t raw_data[14] = {0};                                                 // x/y/z accel register data stored here
        if(read_bytes(mpu_i2c_addr, ACCEL_XOUT_H, 14, &raw_data[0]))             // Read the 14 raw data registers into data array
        {
    ...
    void update_mag() {
        ...
        m[0] = (float)(mag_count[0] * mag_resolution * mag_bias_factory[0] - mag_bias[0]) * mag_scale[0];  // get actual magnetometer value, this depends on scale being set
        m[1] = (float)(mag_count[1] * mag_resolution * mag_bias_factory[1] - mag_bias[1]) * mag_scale[1];
    ...
    void read_mag(int16_t* destination) {
        ...
        if(read_byte(AK8963_ADDRESS, AK8963_ST1) & 0x01){                   // wait for magnetometer data ready bit to be set
            if(read_bytes(AK8963_ADDRESS, AK8963_XOUT_L, 7, &raw_data[0]))     // Read the six raw data and ST2 registers sequentially into data array
            {
    ...
    void calibrate_acc_gyro_impl() {
        ...
        delay(100);
        initMPU9250();
        saveAccelBias(acc_bias);
        saveGyroBias(gyro_bias);
        delay(1000);
    }
    ...
    void collect_acc_gyro_data_to(float* a_bias, float* g_bias) {
        ...
        for (uint16_t ii = 0; ii < packet_count; ii++) {
            ...
            if(read_bytes(mpu_i2c_addr, FIFO_R_W, 12, &data[0]))              // read data for averaging
            {
    ...
    bool self_test_impl()  // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
    {
        ...
        for (int ii = 0; ii < 200; ii++) {  // get average current values of gyro and acclerometer
            ...
            if(read_bytes(mpu_i2c_addr, ACCEL_XOUT_H, 6, &raw_data[0]))          // Read the six raw data registers into data array
            {
                ...
                aCount--;
            if(read_bytes(mpu_i2c_addr, GYRO_XOUT_H, 6, &raw_data[0]))          // Read the six raw data registers sequentially into data array
            {
    ...
    uint8_t read_byte(uint8_t address, uint8_t subAddress) {
        ...
        bcm2835_i2c_setSlaveAddress(address);
        sendBuf[0] = subAddress;
        if((errCode = bcm2835_i2c_read_register_rs(sendBuf,reinterpret_cast<char*>(&data),1)))
        {
    ...
    bool read_bytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t* dest) {
        bcm2835_i2c_setSlaveAddress(address);
        sendBuf[0] = subAddress;
        if((errCode = bcm2835_i2c_read_register_rs(sendBuf,reinterpret_cast<char*>(dest),count)))
        {
            printf("bcm2835_i2c_read_register_rs failed at %s%d, errCode = 0x%x\n",__FILE__,__LINE__,errCode);
            return false;
        }else
